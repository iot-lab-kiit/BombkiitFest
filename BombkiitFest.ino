...
extern int remainingSeconds = 900;
bool gameOver = false;
...
      if (Task4Handle != NULL) vTaskDelete(Task4Handle);
      if (Task5Handle != NULL) vTaskDelete(Task5Handle);
      if (Task6Handle != NULL) vTaskDelete(Task6Handle);
      if (Task7Handle != NULL) vTaskDelete(Task7Handle);
      if (Task8Handle != NULL) vTaskDelete(Task8Handle);
    }
  }
...
  sendMQTTMessage(bomb, 0, remainingSeconds);  // send the start time to the server
...
      levelCompleted = true;
      sendMQTTMessage(bomb, (int)1, remainingSeconds);
    }
...
      if (targetDistanceReached && currentMillis - distanceStartTime >= 3000)  // 5 seconds
         {
          Serial.println("Distance met for 5 seconds!");
          levelCompleted = true;
          levelDisplay.updateLevel(2);
          sendMQTTMessage(bomb, 2, remainingSeconds);
        }
...
        sendMQTTMessage(bomb, (int)3, remainingSeconds);
      }
    }
    vTaskDelay(100 / portTICK_PERIOD_MS);
  }
...
      sendMQTTMessage(bomb, (int)4, remainingSeconds);
    }
...
      sendMQTTMessage(bomb, (int)5, remainingSeconds);
    }
    vTaskDelay(100 / portTICK_PERIOD_MS);
    //delay(100);
    
...
      sendMQTTMessage(bomb, 6, remainingSeconds);
    }
    vTaskDelay(100 / portTICK_PERIOD_MS);
  }
...
        sendMQTTMessage(bomb, (int)7, remainingSeconds);
      }
...
      digitalWrite(LED_WIN, HIGH);
      sendMQTTMessage(bomb, 8, remainingSeconds);
